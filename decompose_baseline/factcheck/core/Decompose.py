    ...
    def getclaims(self, doc: str, num_retries: int = 3, prompt: str = None) -> list[str]:
        ...
        for i in range(num_retries):
            ...
            try:
                ...
                if json_match:
                    claims = eval(json_match.group())["claims"]
    ...
    def restore_claims(self, doc: str, claims: list, num_retries: int = 3, prompt: str = None) -> dict[str, dict]:
        ...
        for i in range(num_retries):
            ...
            try:
                ...
                claim2doc = json.loads(json_response)
                assert len(claim2doc) == len(claims)
                claim2doc_detail, flag = restore(claim2doc)

                # Add translation logic here
                #for claim, details in claim2doc_detail.items():
                #    translated_claim = self.translate_to_czech(claim)  # Call translation function
                #    details["translated_claim"] = translated_claim  # Add translated claim to the details
                ...
                logger.error(f"JSON Decode Error: {e}, response is: {response}")
            except Exception as e:
                logger.error(f"")#logger.error(f"Parse LLM response error {e}, response is: {response}")
                #logger.error(f"Parse LLM response error, prompt is: {messages}")
